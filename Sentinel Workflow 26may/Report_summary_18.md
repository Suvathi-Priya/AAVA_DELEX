...
|---|---|---|---|---|---|---|---|
| DEF_BRZ-001_002 | UT_BRZ-001_002 | BRZ-001 | NULL | Validate pipeline behavior when the Self-Hosted Integration Runtime is offline. Pipeline failed to send an alert and prevent partial data write. | connectivity | high | open |
| DEF_BRZ-001_009 | UT_BRZ-001_009 | BRZ-001 | NULL | Validate retry logic when network failure persists. Pipeline did not retry the required number of times before failure. | policy_enforcement | high | open |
...
The overall coverage rate is 70.00%, the overall execution success rate is 86.70%, and the recorded defect severity includes high-severity open defects. Based on these reported values, the unit test suite reflects Amber coverage status with open defect conditions present.

Conclusion Statement:

The current unit test suite shows partial coverage and open high-severity defects. Remediation is required before progression.
